...
DaVinci Resolve (Studio) on Linux — install notes, gotchas, and (planned) AI-driven
control via a modified MCP server. Split out (2026-08-24) from an unrelated VR headset
project (`reverb-g2`) on the same rig, where this had been accumulating as a side note.
...
- **Installer**: `DaVinci_Resolve_Studio_21.0.4_Linux.zip`, downloaded from Blackmagic's
  ...
  the `.zip` itself already deleted once `unzip` succeeded, freed 11GB back).
...
- **`.deb` build in progress** as of this writing.
...
## AI-driven control via MCP — planned, expect heavy modification
...
to an MCP client, and is confirmed to support Linux + Resolve Studio 18.5+ via the app's
...
**The plan is not to use it as-is.** First look at its actual code suggests this will
need substantial changes for what this project wants from it — not scoped in detail
yet, but expect a real fork/heavy modification here, not a thin config wrapper around
the upstream project. Not installed or evaluated in depth yet as of this writing; that
evaluation is the next real step once Resolve itself is confirmed working.
